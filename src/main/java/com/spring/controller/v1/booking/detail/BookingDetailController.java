...
public class BookingDetailController {
	...
	@PostMapping()
	public ResponseEntity<Response<BookingDetailDTO>> create(@RequestBody @Valid BookingDetailDTO bookingDetailDTO,
	...
	@PutMapping("{id}")
	public ResponseEntity<Response<BookingDetailDTO>> update(@PathVariable("id") Long id,
			@RequestBody @Valid BookingDetailDTO bookingDetailDTO, BindingResult bindingResult) {
		Response<BookingDetailDTO> response = new Response<>();
		if (bindingResult.hasErrors()) {
			bindingResult.getAllErrors().forEach(error -> response.addErrorMsgToResponse(error.getDefaultMessage()));
			return ResponseEntity.badRequest().body(response);
