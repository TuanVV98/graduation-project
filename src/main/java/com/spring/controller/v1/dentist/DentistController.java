...
//    })
    @GetMapping("")
    public ResponseEntity<List<DentistProfileDTO>> getAllDentist() {
...
//    })
    @PostMapping("")
    public ResponseEntity<Response<DentistProfileDTO>> createDentist(@Valid @RequestBody DentistProfileDTO dto, BindingResult result) throws NotFoundException {
...
//    })
    @PutMapping("/{id}")
    public ResponseEntity<Response<DentistProfileDTO>> updateDentist(@PathVariable("id") Long id, @Valid @RequestBody DentistProfileDTO dto, BindingResult result) throws NotFoundException {
        Response<DentistProfileDTO> response = new Response<>();
        if (result.hasErrors()) {
            result.getAllErrors().forEach(error -> response.addErrorMsgToResponse(error.getDefaultMessage()));
            return ResponseEntity.badRequest().body(response);
