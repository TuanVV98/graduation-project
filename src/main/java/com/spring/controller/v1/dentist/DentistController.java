...
public class DentistController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<DentistProfileDTO> getDentist(@PathVariable("id") Long id){
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Response<DentistProfileDTO>> updateDentist(@PathVariable("id") Long id, @Valid @RequestBody DentistProfileDTO dto, BindingResult result){
        Response<DentistProfileDTO> response = new Response<>();
        if(result.hasErrors()){
            result.getAllErrors().forEach(error -> response.addErrorMsgToResponse(error.getDefaultMessage()));
            return ResponseEntity.badRequest().body(response);
